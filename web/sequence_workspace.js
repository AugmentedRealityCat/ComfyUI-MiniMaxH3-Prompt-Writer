...
export function createSequenceWorkspace(host) {
  ...
  left.innerHTML = `<div class="h3ps-control-grid"><label class="h3ps-field h3ps-duration-field" title="Applies to the next Add chunk; existing chunks keep their durations"><span>New chunk duration <b data-seq-default-label></b></span><div><input type="range" min="1" max="${MAX_CHUNK_DURATION}" step="1" data-seq-default><i></i></div></label>${aspectRatioMarkup(host.icon,"sequence-aspect")}</div>
    ...
    <label class="h3ps-brief"><span><strong>Creative brief</strong><small>Describe the whole sequence, including absolute times</small></span><textarea spellcheck="false" data-seq-brief placeholder="At around 15 seconds she stands up…"></textarea><small class="h3ps-char-count" data-seq-brief-count></small></label>
  ...
  const aspectControl = bindAspectRatio(left.querySelector(".h3ps-choice"), state.aspectRatio, value => { state.aspectRatio = value; persist(); });
